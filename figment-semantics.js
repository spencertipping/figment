...
//   hierarchy. (This concern is obviously subject to any macros that might fix the problem by rearranging stuff.)
  ...
  where*[qualifies_for_regexp_promotion(t) = t.data === 'join' && t[0].match(qs[r]) && t[1].is_string(),
         is_valid_braced_group(t)          = t.data === '{' && l*[valid_kv_pair(t)        = t.data === ':' && t[0].length === 0,
                                                                  valid_braced_element(t) = t.data === ',' ? valid_kv_pair(t[0]) && valid_braced_element(t[1]) :
         ...
         regexp_promotion(t)               = qualifies_for_regexp_promotion(t) && qs[_x] /se[_.data = t[1].data.replace(/\//g, '\\$1') /re['/#{_.substring(1, _.length - 1)}/']],
         constant_literal(t)               = t.is_constant() && t,
         dot(t)                            = t.length === 2 && t.data === '.' && t,
         braced_group(t)                   = is_valid_braced_group(t) ? t : qs[_x['{}']()].replace({_x: transform(t[0])}),
         direct_join_as_invocation(t)      = is_valid_invocation_target(t[1]) && qs[_x(_y)].replace({_x: t[0], _y: t[1][0]})m,
         join_to_invocation_promotion(t)   = qs[_x(_y)].replace({_x: t[0], _y: t[1]}),
         ...
         identifier(t)                     = t.replace(/[^A-Za-z0-9_$]/g, _) /cps.c[identifier_mapping[c] || '$#{c.charCodeAt(0).toString(16)}'] /re[/\d/.test(_.charAt(0)) ? '$#{_}' : _],
         ...
         transform(t)                      = transform_single(t.map(transform))]});
// Generated by SDoc 
