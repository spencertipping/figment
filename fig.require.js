...
// things is the filesystem; loading files over HTTP isn't supported yet. Also, because I'm being lazy, files are loaded synchronously rather than using CPS.
  ...
  caterwaul.tconfiguration('std seq continuation', 'fig.require.nodejs', (function (require) {return fn_[
    this.method('figment_require', fn[files, cc][cc(seq[~files.split(/\s+/) *[require('fs').readFileSync(_, 'utf8')]].slice())])]})(typeof require === 'undefined' || require)).
...
// The callback function isn't run through caterwaul. This gives it the ability to refer to closure state.
  ...
  tconfiguration('std seq continuation', 'fig.require.ajax', function () {
    this.method('figment_require', fn[files, cc][
      l*[file_list = seq[~files.split(/\s+/)], contents = {}, requests_left = file_list.size(), receive(filename)(data) = --requests_left /se[contents[filename] = data, _ || got_everything()]
         got_everything() = cc(seq[file_list *[contents[_]]])] in
      seq[file_list *![get(_, receive(_))]]]),

    where*[create_xhr()      = window.XMLHttpRequest /re[_ ? new _() : new window.ActiveXObject('Microsoft.XMLHTTP')],
           get(url, success) = create_xhr() /se[_.open('GET', url, true), _.onreadystatechange() = _.readyState === 4 && success(_.responseText)]]}).
...
// caterwaul function should already be configured as a figment parser for this to work. (The 'fig' configuration takes care of this for you.)
  ...
  tconfiguration('std seq continuation', 'fig.require', function () {
    ...
    method('require', fn[modules, cc][this.figment_require(modules, _) /cpb.ms[cc && cc.call(this /se[seq[~ms *![cc(_)]]])]])});
// Generated by SDoc 
