  ...
  caterwaul.tconfiguration('std seq continuation parser', 'figment', function () {
    ...
    where*[parse(s)      = expression(lex(s)),
           ...
           atom          = l*[quoted_operator = peg[c('_') % operator >> fn[xs][xs.join('')]], number_options = peg[c(/\d+/, 1) / c(/\d*\.\d+([eE][-+]?\d+)?/, 1) >> fn[xs][xs[0]]],
                                                                                               string_options = peg[c(/'([^']|\\.)*'/, 1) / c(/"([^"]|\\.)*"/, 1) >> fn[xs][xs[0]]]] in
           ...
           binary(op, l, inductive, base) = l*[p(x) = p(x), p = peg[l % [op % p] >> fn[xs][xs[1] ? inductive(xs[0], xs[0][1], xs[1][1]) : base ? base(xs[0]) : xs[0]]]] in p,
           prefix(op, l, inductive, base) = l*[p(x) = p(x), p = peg[(op % p >> fn[xs][inductive(xs[0], xs[1])]) / (l >> fn[xs][base ? base(xs[0]) : xs[0]])]] in p,
           ...
           loose_prefix  = peg[prefix(seq(operator, space),      loose_join,   fn[   op, r][new caterwaul.syntax(op[1], r)])],
